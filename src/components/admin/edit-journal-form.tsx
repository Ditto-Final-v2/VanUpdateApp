...
export function EditJournalForm({ post }: { post:AdminPost }) {
  ...
  async function submit(event:React.FormEvent<HTMLFormElement>){event.preventDefault();setError("");setUploading(true);const form=event.currentTarget;
    try { const supabase=createClient();const batch=crypto.randomUUID();const paths:string[]=[];
      for(const file of files){const prepared=await preparePhoto(file);const path=`staged/${batch}/${crypto.randomUUID()}.${prepared.extension}`;const {error:uploadError}=await supabase.storage.from("trip-photos").upload(path,prepared.blob,{contentType:prepared.type});if(uploadError){if(paths.length)await supabase.storage.from("trip-photos").remove(paths);setError(uploadError.message);return;}paths.push(path);}
    ...
    } catch(uploadFailure){setError(uploadFailure instanceof Error?uploadFailure.message:"The photos could not be prepared.");} finally {setUploading(false);}
  }
  ...
  return <form onSubmit={submit} className="mt-8 grid gap-5 border-2 border-forest bg-white p-5 shadow-[5px_5px_0_#1f352d] sm:grid-cols-2 sm:p-7"><input type="hidden" name="postId" value={post.id}/>
    ...
    <Field name="entryLocationName" label="Location the Entry Was Made" defaultValue={post.entryLocationName} required/>
    <Field name="activityLocationName" label="Location of Main Activities" defaultValue={post.activityLocationName} required/>
    <LocationFields latitude={post.latitude} longitude={post.longitude} loopNumber={post.loopNumber}/><Field name="vanMileage" label="Current Van Mileage" type="number" min="0" step="1" defaultValue={post.vanMileage} required/>
    ...
    <fieldset className="border-2 border-dashed border-sage p-4 sm:col-span-2"><legend className="px-2 text-sm font-bold uppercase text-forest">Manage photos</legend>
      {post.photos.length>0&&<div className="mb-4 grid gap-3 sm:grid-cols-2">{post.photos.map((photo)=><div key={photo.path} className={`border-2 p-3 ${retained.has(photo.path)?"border-stone-300":"border-red-700 opacity-50"}`}><input type="hidden" name="photoDetailPath" value={photo.path}/><div className="relative aspect-[4/3]"><Image src={photo.url} alt={photo.alt} fill className="object-cover"/></div><label className="mt-2 flex gap-2 text-xs font-bold"><input type="checkbox" checked={retained.has(photo.path)} onChange={(e)=>toggle(photo.path,e.target.checked)}/>Keep photo</label><label className="mt-1 flex gap-2 text-xs font-bold"><input type="radio" name="coverChoice" checked={cover===photo.path} disabled={!retained.has(photo.path)} onChange={()=>setCover(photo.path)}/>Main cover</label><label className="form-label mt-3">Alt text<input className="form-input mt-1 normal-case" name="photoAlt" defaultValue={photo.alt} maxLength={200} required/></label><label className="form-label mt-2">Caption<input className="form-input mt-1 normal-case" name="photoCaption" defaultValue={photo.caption} maxLength={500}/></label></div>)}</div>}
      ...
      {previews.length>0&&<div className="mt-4 grid gap-3 sm:grid-cols-2">{previews.map((src,index)=><div key={src} className="border-2 border-stone-300 p-3"><span className="relative block aspect-[4/3]"><Image src={src} alt={`New photo ${index+1}`} fill unoptimized className="object-cover"/></span><label className="mt-2 flex gap-2 text-xs font-bold"><input type="radio" name="coverChoice" checked={cover===`new:${index}`} onChange={()=>setCover(`new:${index}`)}/>Main cover</label><label className="form-label mt-3">Alt text<input className="form-input mt-1 normal-case" name="newPhotoAlt" defaultValue={`Journal photo ${index+1}`} maxLength={200} required/></label><label className="form-label mt-2">Caption<input className="form-input mt-1 normal-case" name="newPhotoCaption" maxLength={500}/></label></div>)}</div>}
