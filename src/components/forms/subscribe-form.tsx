...
export function SubscribeForm({ compact = false }: { compact?: boolean }) {
  ...
  return <form action={action} noValidate className={compact ? "space-y-3" : "grid gap-4 sm:grid-cols-[1fr_1.3fr_auto] sm:items-end"}>
    ...
    <button className="button-primary h-[50px]" type="submit" disabled={pending}>{pending?"Saving…":"Keep me posted"}</button>
