...
export function SubscribeForm({ compact = false }: { compact?: boolean }) {
  ...
  return <form action={action} noValidate className={compact ? "space-y-3" : "grid gap-4 sm:grid-cols-2 sm:items-end"}>
    ...
    <div><label htmlFor={compact ? "prompt-phone" : "subscribe-phone"} className="form-label">Mobile number</label><input id={compact ? "prompt-phone" : "subscribe-phone"} name="phone" type="tel" required autoComplete="tel" inputMode="tel" className="form-input" placeholder="(915) 555-0123" /></div>
    <label className="flex items-start gap-2 text-[.7rem] leading-5 text-stone-700 sm:col-span-full"><input name="smsConsent" type="checkbox" required className="mt-1 h-4 w-4 shrink-0" /><span>Send me recurring automated Road &amp; Country journal alerts by text/MMS. Message frequency varies. Message and data rates may apply. Reply STOP to unsubscribe or HELP for help. Consent is not a condition of purchase.</span></label>
