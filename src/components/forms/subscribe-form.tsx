...
export function SubscribeForm({ compact = false }: { compact?: boolean }) {
  ...
  if (state.success) return <div role="status" className="feedback-success flex items-start gap-3 p-4"><CheckCircle2 className="feedback-icon mt-0.5 shrink-0" /><div><p className="font-bold">You’re on the list.</p><p className="feedback-detail mt-1 text-sm">{state.message}</p></div></div>;
  return <form action={action} noValidate className={compact ? "space-y-3" : "grid gap-4 sm:grid-cols-2 sm:items-end"}>
    ...
    {state.message && <p id="subscribe-error" role="alert" className="feedback-error p-3 text-sm font-bold sm:col-span-full">{state.message}</p>}
  </form>;
}
