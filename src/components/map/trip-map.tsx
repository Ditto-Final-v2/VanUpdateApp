...
export function TripMap({ posts, compact = false, center,tripState }: TripMapProps) {
  const container = useRef<HTMLDivElement>(null); const fallbackMap = useRef<HTMLDivElement>(null); const map = useRef<maplibregl.Map | null>(null); const vanArt = useRef<HTMLSpanElement>(null); const fallbackView = useRef({ scale: 1, x: 0, y: 0 }); const fallbackDrag = useRef<{ pointerId: number; x: number; y: number } | null>(null); const [selectedPost, setSelectedPost] = useState<TripPost | null>(null); const [selectedLoop, setSelectedLoop] = useState<LoopId>(tripState?.activeLoop??1); const [mapReady, setMapReady] = useState(false); const [mapError, setMapError] = useState<string | null>(null);
  useEffect(() => {
    if (!container.current || map.current) return;
    let instance: maplibregl.Map;
    try {
      instance = new maplibregl.Map({ container: container.current, style: siteConfig.mapStyleUrl, center: center ?? [-109.5, 36], zoom: compact ? 8 : 4.6, minZoom: 2.5, maxZoom: 14, attributionControl: false, cooperativeGestures: false, dragRotate: false, pitchWithRotate: false, touchPitch: false });
    } catch {
    ...
    const handleFallbackPointerDown = (event: PointerEvent) => {
      if (fallbackView.current.scale <= 1 || event.button > 0 || (event.target as HTMLElement).closest("button, a, .maplibregl-control-container")) return;
    ...
    return () => { window.clearTimeout(loadTimeout); mapSurface.removeEventListener("wheel", handleWheelZoom, { capture: true }); mapSurface.removeEventListener("pointerdown", handleFallbackPointerDown); mapSurface.removeEventListener("pointermove", handleFallbackPointerMove); mapSurface.removeEventListener("pointerup", handleFallbackPointerUp); mapSurface.removeEventListener("pointercancel", handleFallbackPointerUp); instance.remove(); map.current = null; };
  ...
  return <div className={`trip-map-shell relative overflow-hidden ${compact ? "trip-map-shell-compact h-72 rounded-3xl bg-[#d9ddd5]" : "trip-map-shell-main h-[62vh] min-h-[500px] max-h-[760px] rounded-[1.75rem] bg-[#d6e2e3] sm:min-h-[560px]"}`}>
    ...
    {!compact && <div aria-hidden="true" className="map-gesture-hint">Drag to move · Pinch or scroll to zoom</div>}
