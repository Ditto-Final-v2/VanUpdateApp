...
function interactiveLabelsMarkup(markup:string){let index=0;return markup.replaceAll("<text>",()=>`<text class="map-destination-label" data-stop-index="${index++}" role="button" tabindex="0">`);}
function interactiveLabelIndex(target:EventTarget){if(!(target instanceof Element))return null;const label=target.closest(".map-destination-label");if(!label)return null;const index=Number(label.getAttribute("data-stop-index"));return Number.isInteger(index)?index:null;}
...
function TripRouteOverlay({posts,selectedLoop,liveState,onSelectStop}:{posts:TripPost[];selectedLoop:LoopId;liveState?:TripMapState;onSelectStop:(place:SelectedPlace)=>void}){
  ...
  const openStop=(stop:OverlayStop)=>onSelectStop({name:stop.name,posts:entriesByStop.get(stop.name)??[]});
  const openLabel=(target:EventTarget)=>{const index=interactiveLabelIndex(target);const stop=index===null?undefined:active.stops[index];if(!stop)return false;openStop(stop);return true;};
  return <svg className="absolute inset-0 h-full w-full overflow-visible" viewBox="0 0 1400 760" preserveAspectRatio="xMidYMid meet" role="img" aria-label={`Loop ${selectedLoop} planned route with completed travel and current van location`}>
    <style>{`.svg-trip-route{fill:none;stroke-linecap:round;stroke-linejoin:round;stroke-dasharray:8 7}.svg-trip-labels text{fill:#263f37;stroke:#fffdf8;stroke-width:4px;paint-order:stroke;stroke-linejoin:round;font:750 15px system-ui,sans-serif}.svg-trip-labels .leader{fill:none;stroke:#71806a;stroke-width:1.5}.svg-trip-labels .map-destination-label{pointer-events:all;cursor:pointer;outline:none;transition:fill .15s,stroke-width .15s}.svg-trip-labels .map-destination-label:hover,.svg-trip-labels .map-destination-label:focus{fill:#7c3aed;stroke-width:5px}.map-destination{pointer-events:all;cursor:pointer;outline:none}.map-destination .stop-dot{transition:r .15s,stroke-width .15s}.map-destination:hover .stop-dot,.map-destination:focus .stop-dot{r:8px;stroke-width:4px}`}</style>
    ...
    <g className="svg-trip-labels" dangerouslySetInnerHTML={{__html:interactiveLabelsMarkup(active.labelsMarkup)}} onPointerDown={(event)=>{if(interactiveLabelIndex(event.target)!==null)event.stopPropagation();}} onClick={(event)=>openLabel(event.target)} onKeyDown={(event)=>{if((event.key==="Enter"||event.key===" ")&&openLabel(event.target)){event.preventDefault();event.stopPropagation();}}}/>
    <g>{active.stops.map((stop)=>{
      const entries=entriesByStop.get(stop.name)??[];
      const open=()=>openStop(stop);
...
export function TripMap({ posts, compact = false, center,tripState }: TripMapProps) {
  ...
  return <div className={`trip-map-shell relative overflow-hidden ${compact ? "trip-map-shell-compact h-72 rounded-3xl bg-[#d9ddd5]" : "trip-map-shell-main h-[62vh] min-h-[500px] max-h-[760px] rounded-[1.75rem] bg-[#d6e2e3] sm:min-h-[560px]"}`}>
    ...
    {!compact && <div aria-hidden="true" className="map-gesture-hint"><span className="map-tip-desktop">Drag to move · Scroll to zoom · Click a dot or label for entries</span><span className="map-tip-mobile">Two fingers move or zoom · Tap a dot or label for entries · One finger scrolls</span></div>}
