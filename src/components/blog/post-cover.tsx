...
export function PostCover({ src, alt, priority = false, className = "", fit = "cover" }: { src: string | null; alt: string; priority?: boolean; className?: string; fit?: "cover" | "contain" }) {
  ...
  if (fit === "contain") return <>
    <Image src={src} alt="" aria-hidden="true" fill priority={priority} unoptimized sizes="(max-width: 1024px) 100vw, 60vw" className="scale-110 object-cover opacity-50 blur-2xl" />
    <span aria-hidden="true" className="absolute inset-0 bg-[#172d28]/20" />
