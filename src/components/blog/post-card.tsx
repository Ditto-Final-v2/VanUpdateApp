...
export function PostCard({ post }: { post: TripPost }) {
  return <article className="retro-post-card group overflow-hidden bg-white">
    <Link href={`/journal/${post.slug}`} className="block overflow-hidden focus-ring">
      <Image src={post.coverImage} alt={post.coverImageAlt} width={800} height={520} className="aspect-[4/3] w-full object-cover transition duration-500 group-hover:scale-[1.025]" />
    </Link>
