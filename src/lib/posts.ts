...
type PostRow = {
  ...
  status: "published"; trip_day: number; mileage_to_date: number;
  post_photos: PhotoRow[] | null;
};

const postColumns = "id,slug,title,excerpt,body,entry_date,published_at,location_name,latitude,longitude,cover_image_path,cover_image_alt,notification_title,status,trip_day,mileage_to_date,post_photos(storage_path,alt_text,caption,sort_order)";
...
async function addSignedPhotos(rows: PostRow[]): Promise<TripPost[]> {
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      tripDay: row.trip_day, mileageToDate: row.mileage_to_date,
    };
  });
}
...
const adminColumns = "id,title,entry_date,location_name,van_mileage,miles_walked,miles_ran,miles_biked,major_cities_visited,new_states_visited,new_national_parks_visited,tanks_of_gas,notification_title,body,status,cover_image_path,post_photos(storage_path,alt_text,sort_order)";
...
export async function getAdminPostById(id: string): Promise<AdminPost | null> {
  ...
  const row = data as unknown as {
    id:string; title:string; entry_date:string; location_name:string; van_mileage:number;
  ...
  return { id:row.id,title:row.title,entryDate:row.entry_date,locationName:row.location_name,vanMileage:row.van_mileage,
